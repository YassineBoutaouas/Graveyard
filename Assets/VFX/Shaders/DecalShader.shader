Shader "Universal Render Pipeline/NiloCat Extension/Screen Space Decal/Unlit"
{
    Properties
    {
        ...
        [HDR]_Color("Color", color) = (1,1,1,1) //(default = 1,1,1,1)

        [Header(Blending)]
        // https://docs.unity3d.com/ScriptReference/Rendering.BlendMode.html
        [Enum(UnityEngine.Rendering.BlendMode)]_SrcBlend("SrcBlend", Float) = 5 // 5 = SrcAlpha /*(default = SrcAlpha)*/
        [Enum(UnityEngine.Rendering.BlendMode)]_DstBlend("DstBlend", Float) = 10 // 10 = OneMinusSrcAlpha /*(default = OneMinusSrcAlpha)*/

        [Header(Alpha remap(extra alpha control))]
        _AlphaRemap("AlphaRemap", vector) = (1,0,0,0) // (default = 1,0,0,0) _____alpha will first mul x, then add y    (zw unused)

        [Header(Prevent Side Stretching(Compare projection direction with scene normal and Discard if needed))]
        [Toggle(_ProjectionAngleDiscardEnable)] _ProjectionAngleDiscardEnable("ProjectionAngleDiscardEnable", float) = 0 // (default = off)
        _ProjectionAngleDiscardThreshold("ProjectionAngleDiscardThreshold", range(-1,1)) = 0 // (default = 0)

        [Header(Mul alpha to rgb)]
        [Toggle]_MulAlphaToRGB("MulAlphaToRGB", Float) = 0 // (default = off)

        [Header(Ignore texture wrap mode setting)]
        [Toggle(_FracUVEnable)] _FracUVEnable("FracUVEnable", Float) = 0 // (default = off)
        ...
        [Enum(UnityEngine.Rendering.CompareFunction)]_StencilComp("StencilComp", Float) = 0 //0 = disable // (default = Disable) _____Set to NotEqual if you want to mask by specific _StencilRef value, else set to Disable
        ...
        [Enum(UnityEngine.Rendering.CompareFunction)]_ZTest("ZTest", Float) = 0 //0 = disable // (default = Disable) _____to improve GPU performance, Set to LessEqual if camera never goes into cube volume, else set to Disable
        ...
        [Enum(UnityEngine.Rendering.CullMode)]_Cull("Cull", Float) = 1 //1 = Front // (default = Front) _____to improve GPU performance, Set to Back if camera never goes into cube volume, else set to Front

        [Header(Unity Fog)]
        [Toggle(_UnityFogEnable)] _UnityFogEnable("UnityFogEnable", Float) = 1 // (default = on)

        [Header(Support Orthographic camera)]
        [Toggle(_SupportOrthographicCamera)] _SupportOrthographicCamera("SupportOrthographicCamera", Float) = 0 // (default = off)
    }
